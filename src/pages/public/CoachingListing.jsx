...
const CoachingCard = ({ profile }) => (
    <Link to={`/coachings/${profile.user._id}`}
        ...
        <div className="bg-gradient-to-r from-blue-900 to-indigo-900 p-5 flex items-center gap-4">
            <img
                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(profile.user.name)}&size=80&background=e0e7ff&color=3730a3&bold=true&rounded=true`}
            ...
            <div className="min-w-0">
                ...
                {profile.user.subscriptionStatus === 'active' && (
                    <span className="text-[9px] font-black uppercase bg-amber-400 text-amber-900 px-2 py-0.5 rounded-full mt-1 inline-block">⭐ Premium</span>
                )}
            </div>
        </div>
        <div className="p-5">
            <div className="flex flex-wrap gap-1.5 mb-4">
                {(profile.courses || []).slice(0, 3).map(c => (
                    <span key={c} className="bg-indigo-50 text-indigo-700 text-[10px] font-bold px-2.5 py-1 rounded-lg">{c}</span>
                ))}
            </div>
            {profile.bio && (
                <p className="text-slate-500 text-sm line-clamp-2 mb-4 leading-relaxed">{profile.bio}</p>
            )}
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <div className="bg-gradient-to-r from-blue-900 to-indigo-900 pt-20">
                <div className="page-container py-10">
                    <h1 className="text-3xl font-black text-white mb-1">Coaching Institutes</h1>
            ...
            <div className="page-container py-8">
                ...
                {showFilters && (
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6">
                        <div className="grid md:grid-cols-2 gap-5">
                            ...
                            <div>
                                <label className="text-sm font-bold text-slate-700 mb-2 block">Course</label>
                                <input className="input-field" placeholder="e.g. JEE Mains, NEET..."
                                    value={filters.course} onChange={e => setFilters(f => ({ ...f, course: e.target.value }))} />
                            </div>
                        </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
                        ...
                        <p className="text-slate-700 font-bold text-lg mb-1">No institutes found</p>
